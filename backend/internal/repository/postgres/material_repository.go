...
func (r *MaterialRepository) Create(ctx context.Context, m *material.Material) error {
	const q = `
		INSERT INTO materials (lesson_id, file_name, file_path, uploaded_at)
		VALUES ($1, $2, $3, $4)
		RETURNING id`
	var id uint
	err := r.pool.QueryRow(ctx, q, m.LessonID(), m.FileName(), m.FilePath(), m.UploadedAt()).Scan(&id)
	if err != nil {
...
func (r *MaterialRepository) ListByLesson(ctx context.Context, lessonID uint) ([]*material.Material, error) {
	const q = `SELECT id, lesson_id, file_name, file_path, uploaded_at FROM materials WHERE lesson_id = $1 ORDER BY uploaded_at ASC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &lID, &fileName, &filePath, &uploadedAt); err != nil {
			return nil, fmt.Errorf("postgres: đọc dòng material lỗi: %w", err)
		}
		result = append(result, material.Rehydrate(id, lID, fileName, filePath, uploadedAt))
	}
